...
let {videos,loading:relatedVideosLoading} = useSelector(state=>state.relatedVideo)
const {video,loading} = useSelector(state=>state.selectedVideo)

let _videos = videos
videos = []
for (let i = 0; i < _videos.length; i++) {
    if(_videos[i].snippet){
  ...
  return (
    <Row>
        ...
        <Col lg={4}>
            {!relatedVideosLoading ? videos?.map((video)=><VideoHorizontal video={video} key={video.id.videoId}/>):
